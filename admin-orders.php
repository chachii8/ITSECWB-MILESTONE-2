...
if (isset($_POST['update_status'])) {
    ...
    if (!validate_order_status($status) || $orderId < 1) {
        // Invalid - do nothing, page will reload without update
    } elseif (!can_update_order_status($conn, $orderId)) {
        // Order already Delivered or Cancelled - no changes allowed
    } else {
        $statusUpdated = updateOrderStatus($orderId, $status);

        // If updated, show success toast (without page refresh)
        if ($statusUpdated) {
            log_audit(
                $conn,
...
<header class="header">
  ...
  <nav class="navigation">
    ...
    <a href="admin-orders.php" class="nav-link active">ORDERS</a>
